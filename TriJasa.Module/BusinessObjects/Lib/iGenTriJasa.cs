   ...
   public class iGenTriJasa : iGetKey
    {
        ...
        public string SMKomisiNomorGet(Session oSession, DateTime oDate)
        {
            string sNumer = "";
            int sRun = 1;
            string sTahunBulan = oDate.ToString("yyMM");
            string sYear = $"KSM{sTahunBulan}.";
        ...
        public TarifSMHarga TarifHarga(Session oSession, DateTime Tgl, Pelanggan oPelanggan)
        {
            //string ReportName = "";

            string sqlQuery = string.Format($" TarifSM.pelanggan.Oid == '{oPelanggan.Oid} && Berlaku <= #{Tgl}# ");
            CriteriaOperator filterOperator = CriteriaOperator.Parse(sqlQuery);
            TarifSMHarga oiHarga = oSession.FindObject<TarifSMHarga>(filterOperator);
        ...
        public void SupirKlaimSet(Session oSession, eSupirKlaim eKlaim, Supir oSupir, DaftarMuatan oDM , double Amount ,string oKeterangan,DateTime odate)
        {
            ...
            if (oSupirKlaim==null)
            {
                ...
                if (onSupir.Group != null)
                {
                    // sqlQuery = string.Format(" Oid == {0} ", onSupir.Group);
                    //CriteriaOperator filterOperator = CriteriaOperator.Parse(sqlQuery);
                    // onSupir = oSession.FindObject<Supir>(filterOperator);

                    oSupirKlaim.Supir = (Supir)onSupir.Group;
                }
                else
                {

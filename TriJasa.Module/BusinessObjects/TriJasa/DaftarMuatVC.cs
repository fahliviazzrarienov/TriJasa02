...
//            TargetViewType = ViewType.ListView;
                    ...
                    && (((DaftarMuatan)View.CurrentObject).NamaSpr != null))
                {
                    if (this.ObjectSpace.IsModified)
                    {
                        try
                        {
                            ...
                            //ListPropertyEditor peListSuratMuatan = ((DaftarMuatan)View.CurrentObject).SuratMuatan as ListPropertyEditor;
                            XPCollection<SuratMuatan> oListSuratMuatan = oSuratMuatan.SuratMuatan;
                            if (oListSuratMuatan.Count > 0)
                            {
                                foreach (SuratMuatan oSM in oListSuratMuatan)
                                {
                                    ...
                                    else if ( oSM.Pelanggan!=null && oSM.Dikerimke!=null)
                                    {
                                        oSM.UpdateJenisBarang(true);
                                        oSM.Save();
                                        oSM.Session.CommitTransaction();
        ...
        private void SaveAction_Executing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            if (View.CurrentObject is DaftarMuatan)
            {
                ...
                DaftarMuatan currentObject = (DaftarMuatan)View.CurrentObject;
                XPCollection<SuratMuatan> oListSuratMuatan = currentObject.SuratMuatan;
                ...
                foreach (SuratMuatan oSM in oListSuratMuatan)
                {
                    ...
                    {
                        //oSM.Delete();
                        currentObject.Session.Delete(oSM);
                    }

    ...
    public class NotaPayment : XPObject
   {
     ...
     protected override void OnDeleting()
     {
       base.OnDeleting();
        ...
        [XafDisplayName("Terakhir Update"), ToolTip("Terakhir Update")]
        [Size(20)]
        //[ImmediatePostData]
     ...
     { 
       ...
       set { SetPropertyValue("Pelanggan", ref _Pelanggan, value); } 
        ...
        private void RefreshAvailableTasks()
        {
            if (availableTasks == null)
                return;
            // string sqlQuery = string.Format($" ([Status] == 3 || [Status] == 5)  && (Sisa!=0 || PmtDiterima !=0) ");// &&  Status != {eTagihan.Kolektor.GetHashCode()}  ");
            // string sqlQuery = string.Format($" ([Status] == 3 || [Status] == 5) ");// &&  Status != {eTagihan.Kolektor.GetHashCode()}  ");
            //string sqlQuery = string.Format($" [Status] In (3,5) ");// &&  Status != {eTagihan.Kolektor.GetHashCode()}  ");
            string sqlQuery = "";
